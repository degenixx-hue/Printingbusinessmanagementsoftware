...
export default function CreateQuotation() {
  ...
  const { clients, products, addQuotation } = useData();
  ...
  const [currentItem, setCurrentItem] = useState({
    ...
    designCount: 52,
    creativePackageType: 'Festive' as 'Festive' | 'Ads',
    creativeDateFrom: '',
    creativeDateTo: '',
    pricePerUnit: 0,
  });
  ...
  const handleProductChange = (productId: string) => {
    ...
    if (product) {
      setCurrentItem({
        ...
        designCount: product.designCount || 52,
  ...
  const handleAddItem = () => {
    ...
    // Validate creative package fields if digital creative is enabled
    if (currentItem.digitalCreative) {
      if (!currentItem.creativeDateFrom || !currentItem.creativeDateTo) {
        toast.error('Please enter both start and end dates for the creative package');
        return;
      }
      if (new Date(currentItem.creativeDateFrom) > new Date(currentItem.creativeDateTo)) {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    navigate(`/orders/quotation/${quotationId}`);
  };
        ...
        <div className="max-w-6xl mx-auto space-y-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-[#1a1a1a] border border-gray-700 rounded-lg p-6">
              <h3 className="font-bold text-white mb-4 text-lg">Client Information</h3>
              <div>
                <Label htmlFor="clientId" className="text-white font-medium">Select Client *</Label>
                  ...
                  <>
                    <div className="border-t border-white/20 pt-4">
                      ...
                      <div className="grid grid-cols-4 gap-4">
                        <div>
                          <Label htmlFor="coverPageQuantity" className="text-white text-xs">Cover Page</Label>
                          <Input
                        ...
                        <div>
                          <Label htmlFor="innerPageQuantity" className="text-white text-xs">Inner Page</Label>
                          <Input
                    ...
                    <div className="flex items-center space-x-2">
                      <Switch
                        id="digitalCreative"
                        checked={currentItem.digitalCreative}
                        onCheckedChange={(checked) => setCurrentItem({ ...currentItem, digitalCreative: checked })}
                      />
                      <Label htmlFor="digitalCreative" className="text-white">Digital Creative</Label>
                    </div>

                    {/* Creative Package Details */}
                    {currentItem.digitalCreative && (
                      <div className="border-t border-white/20 pt-4 space-y-4">
                        <h4 className="text-sm font-medium text-white">Creative Package Details</h4>
                        <div className="grid grid-cols-4 gap-4">
                          <div>
                            <Label htmlFor="creativePackageType" className="text-white">Package Type *</Label>
                            <Select
                              value={currentItem.creativePackageType}
                              onValueChange={(value: 'Festive' | 'Ads') => setCurrentItem({ ...currentItem, creativePackageType: value })}
                            >
                              <SelectTrigger>
                                <SelectValue />
                              </SelectTrigger>
                          ...
                          <div>
                            <Label htmlFor="designCount" className="text-white">Number of Designs *</Label>
                            <Input
                              id="designCount"
                              type="number"
                              min="1"
                              value={currentItem.designCount}
                              onChange={(e) => setCurrentItem({ ...currentItem, designCount: parseInt(e.target.value) || 52 })}
                              placeholder="e.g., 52"
                            />
                          </div>
                          <div>
                            <Label htmlFor="creativeDateFrom" className="text-white">Date From *</Label>
                            <Input
                              id="creativeDateFrom"
                              type="date"
                              value={currentItem.creativeDateFrom}
                              onChange={(e) => setCurrentItem({ ...currentItem, creativeDateFrom: e.target.value })}
                            />
                          </div>
                          <div>
                            <Label htmlFor="creativeDateTo" className="text-white">Date To *</Label>
                            <Input
                              id="creativeDateTo"
                              type="date"
                              value={currentItem.creativeDateTo}
                              onChange={(e) => setCurrentItem({ ...currentItem, creativeDateTo: e.target.value })}
                            />
                          </div>
                        </div>
                        <div className="p-3 bg-purple-500/20 backdrop-blur-sm border border-purple-400/30 rounded-lg">
                          <p className="text-sm text-purple-200">
                            📦 <strong>Note:</strong> This package includes {currentItem.designCount} designs. Creative packages will be managed in the Packages tab and will not generate job sheets.
                          </p>
                        </div>
                      </div>
                    )}
                    ...
                    >
                      <div className="flex-1">
                        ...
                        <div className="flex gap-4 text-xs text-gray-300 mt-1">
                          ...
                          <div>Creative: {item.digitalCreative ? 'Yes' : 'No'}</div>
                        </div>
                        {item.digitalCreative && item.creativeDateFrom && item.creativeDateTo && (
                          <div className="mt-2 p-2 bg-purple-500/10 border border-purple-400/20 rounded text-xs">
                            <span className="text-purple-300 font-medium">{item.creativePackageType} Package:</span>
                            <span className="text-gray-300 ml-2">
                              {new Date(item.creativeDateFrom).toLocaleDateString()} - {new Date(item.creativeDateTo).toLocaleDateString()}
                            </span>
                          </div>
                        )}
                      </div>
